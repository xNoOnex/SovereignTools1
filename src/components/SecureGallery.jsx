...
export function SecureGallery() {
  ...
  const scanGallery = async () => {
    setLoading(true);
    let found = [];
    const targetFolders = ['DCIM', 'Pictures', 'Download', 'Movies'];

    try {
      await Filesystem.requestPermissions();
    } catch (e) {
      console.warn("Permissions already handled natively");
    }
  ...
  return (
    <div className="p-4 space-y-4 max-w-2xl mx-auto pb-28 select-none">
        ...
        {['All', 'Photos', 'Videos'].map(cat => (
          <button
            ...
            className={`flex-1 py-1.5 text-xs font-bold rounded-xl transition-all ${
              filter === cat ? 'bg-cyan-500 text-black shadow' : 'text-zinc-400 hover:text-white'
            }`}
          >
            {cat}
          </button>
      ...
      <div className="bg-zinc-900 p-4 rounded-3xl border border-zinc-800 min-h-[220px]">
        {loading ? (
          <div className="text-center py-12 text-xs text-cyan-400 animate-pulse font-mono">
            🔍 Automatically indexing device media...
          </div>
        ) : displayedFiles.length === 0 ? (
          <div className="text-center py-12 text-xs text-zinc-500 font-mono">
            No media files detected in DCIM, Pictures, or Downloads.
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-3 max-h-96 overflow-y-auto p-1">
            {displayedFiles.map((item, idx) => (
              <div key={idx} className="bg-black/60 border border-zinc-800 p-3 rounded-2xl flex flex-col justify-between">
                ...
                <span className="text-[9px] text-cyan-400 font-bold uppercase mt-2">{item.path}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <ToolFooter title="Gallery Indexer" details="Auto-scans local physical storage paths via Capacitor Filesystem." disclaimer="Zero telemetry or remote backups." />
    </div>
  );
}
