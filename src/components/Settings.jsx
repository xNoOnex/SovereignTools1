...
export function Settings({ closeSettings, pin, setPin, passcode, setPasscode, accentColor, setAccentColor, textSize, setTextSize, onNavigate }) {
  ...
  // Restored to your original working utility!
  const toggleShield = () => {
  ...
  return (
    <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[9999] p-6 animate-fadeIn flex flex-col overflow-y-auto">
      ...
      <div className="flex items-center justify-between pb-6 border-b border-zinc-800 shrink-0">
        ...
        <button onClick={closeSettings} className="bg-zinc-900 border border-zinc-700 px-5 py-2 rounded-xl text-xs font-bold text-zinc-300 active:scale-95 transition-all">
          CLOSE
        </button>
      </div>
      ...
      <div className="flex flex-col gap-6 py-6 pb-20">
        ...
        {/* MESH PROTOCOL (RESTORED) */}
        <button 
          onClick={() => { closeSettings(); onNavigate('mesh_protocol'); }}
          className="w-full py-4 bg-emerald-900/20 border border-emerald-900/50 rounded-2xl text-xs font-black tracking-widest uppercase text-emerald-400 active:scale-95 transition-all flex items-center justify-center gap-2 shadow-lg"
        >
          <span>📡</span> INITIALIZE MESH PROTOCOL
        </button>

        {/* APP OPERATION MODE (RESTORED) */}
        <div className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-2xl flex flex-col gap-4">
          ...
          <div className="flex gap-2">
             <button onClick={() => switchMode('BASIC')} className={`flex-1 py-3 rounded-xl text-xs font-bold tracking-widest transition-all ${appMode === 'BASIC' ? 'bg-zinc-100 text-black shadow-md' : 'bg-black text-zinc-500 border border-zinc-800'}`}>
               BASIC
             </button>
             <button onClick={() => switchMode('EXPERT')} className={`flex-1 py-3 rounded-xl text-xs font-bold tracking-widest transition-all ${appMode === 'EXPERT' ? 'bg-zinc-100 text-black shadow-md' : 'bg-black text-zinc-500 border border-zinc-800'}`}>
               EXPERT
             </button>
          </div>
        </div>
        ...
        <div className="p-4 bg-zinc-900/50 border border-red-900/30 rounded-2xl flex flex-col gap-4 shadow-inner">
          ...
          <p className="text-[10px] font-mono text-zinc-400 leading-relaxed">
            Entering this PIN on the lock screen acts as a Dead Man's Switch. It triggers a catastrophic, unrecoverable data wipe.
          </p>
          <div className="flex gap-2">
        ...
        <div className="p-4 bg-zinc-900/50 border border-blue-900/30 rounded-2xl flex flex-col gap-4 shadow-inner">
          ...
          <p className="text-[10px] font-mono text-zinc-400 leading-relaxed">
            Entering this PIN silently boots the app into Decoy Mode. Sensitive modules will be hidden or populated with fake, benign data.
          </p>
          <div className="flex gap-2">
        ...
        <div className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-2xl flex flex-col gap-4">
          ...
          <div className="grid grid-cols-4 gap-2">
            {['None', '3', '6', '9'].map(limit => (
              <button key={limit} onClick={() => handleUpdateWipeLimit(limit)} className={`py-3 rounded-xl text-xs font-bold transition-all ${wipeLimit === limit ? 'bg-red-900 text-white border border-red-500' : 'bg-black border border-zinc-800 text-zinc-500 hover:border-zinc-600'}`}>
                {limit}
              </button>
            ))}
          </div>
        ...
        <div className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-2xl flex flex-col gap-4">
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
            {themes.map(t => (
              <button 
                key={t.id}
                onClick={() => { setAccentColor(t.id); localStorage.setItem('sovereign_theme', t.id); }}
        ...
        {/* TEXT SCALE (FIXED 0-2 RANGE) */}
        <div className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-2xl flex flex-col gap-4">
          <h4 className="text-sm font-bold text-zinc-200 uppercase tracking-widest">GLOBAL TEXT SCALE</h4>
          <input 
            type="range" min="0" max="2" step="1" 
            value={textSize} 
            onChange={(e) => { setTextSize(parseInt(e.target.value, 10)); localStorage.setItem('sovereign_text_scale', e.target.value); }} 
        ...
        {/* SCREENSHOT TOGGLE (RESTORED TO SECURITYTOGGLE) */}
        ...
        {/* SUPPORT CREATOR */}
        <button 
          onClick={() => { closeSettings(); onNavigate('support'); }}
