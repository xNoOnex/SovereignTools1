...
export function PrivacyBrowser() {
  ...
  const [inputUrl, setInputUrl] = useState('https://searx.be');
  ...
  const handleNavigate = (e) => {
    ...
    if (!target.startsWith('http://') && !target.startsWith('https://')) {
      if (target.includes('.') && !target.includes(' ')) {
        ...
        target = 'https://searx.be/search?q=' + encodeURIComponent(target);
      }
    }
  ...
  return (
    <div className="p-4 space-y-4 max-w-2xl mx-auto pb-28 select-none">
      ...
      <div className="bg-zinc-900 rounded-3xl border border-zinc-800 overflow-hidden shadow-2xl h-[500px] relative">
        <iframe src={url} title="Privacy Browser View" className="w-full h-full bg-white border-0" sandbox="allow-scripts allow-same-origin allow-forms allow-popups" />
      </div>
