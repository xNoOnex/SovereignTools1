    ...
    <div className="p-4 space-y-4 max-w-2xl mx-auto pb-28 select-none">
      ...
      {activeSubTab === 'pgp' && (
        <div className="space-y-4">
          ...
          {pgpMode === 'keys' && (
            <div className="bg-zinc-900/90 p-4 rounded-2xl border border-zinc-800 space-y-3">
              <div className="flex justify-between items-center">
                ...
                <button
                  onClick={handleGeneratePgpKeys}
                  className="px-3 py-1.5 bg-cyan-500 text-black font-bold text-xs rounded-xl"
                >
                  🔑 Generate New Pair
                </button>
              </div>
              ...
              {myPublicKey ? (
                <div className="space-y-2">
                  <div>
                    <label className="text-[10px] text-zinc-400 font-mono">Public Key (Share with Send Contacts)</label>
                    <textarea
                      readOnly
                      rows={4}
                      value={myPublicKey}
                      className="w-full bg-black border border-zinc-800 rounded-xl p-2 text-[10px] text-emerald-400 font-mono mt-1"
                    />
                  </div>
                </div>
              ) : (
                <div className="text-center py-6 text-xs text-zinc-500">
                  No PGP Keypair found. Tap "Generate New Pair" above.
                </div>
              )}
            </div>
          )}
          ...
          {outputArmor && (
            <div className="bg-black p-3 rounded-2xl border border-cyan-500/50 space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-mono text-cyan-400 uppercase font-bold">Output Payload</span>
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(outputArmor);
                    setStatusMsg('Copied payload to clipboard');
                    setTimeout(() => setStatusMsg(''), 2000);
                  }}
                  className="px-2.5 py-1 bg-zinc-800 text-white text-[10px] rounded-lg font-bold"
                >
                  📋 Copy Text
                </button>
              </div>
              <textarea
                ...
                rows={5}
                value={outputArmor}
                className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-2 text-[10px] text-cyan-300 font-mono focus:outline-none"
              />
            </div>
          )}
        </div>
      ...
      {activeSubTab === 'p2p' && (
        <div className="space-y-4">
          <div className="bg-zinc-900/90 p-4 rounded-2xl border border-zinc-800 space-y-3">
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                className="px-3 py-1.5 bg-emerald-500 text-black font-bold text-xs rounded-xl shadow"
              >
                🔌 Connect Node
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs font-mono">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-black border border-zinc-800 rounded-lg p-2 text-white text-[10px] mt-1"
                />
              </div>
              <div>
          ...
          {/* CHAT TIMELINE */}
          <div className="bg-zinc-950 p-4 rounded-2xl border border-zinc-800 space-y-3 h-64 flex flex-col justify-between">
            <div className="overflow-y-auto space-y-2 pr-1 flex-1">
              {chatLog.map(log => (
                <div key={log.id} className="bg-zinc-900 p-2.5 rounded-xl border border-zinc-800 text-xs font-mono">
                  <div className="flex justify-between text-[9px] text-zinc-500">
                    <span className="text-cyan-400 font-bold">{log.sender}</span>
                    <span>{log.time}</span>
                  </div>
                  <div className="text-white mt-1 font-sans">{log.text}</div>
                </div>
              ))}
            </div>

            <form onSubmit={handleSendP2PMsg} className="flex space-x-2 pt-2 border-t border-zinc-800">
              <input
                type="text"
                value={inputMsg}
                onChange={e => setInputMsg(e.target.value)}
                placeholder="Type onion payload message..."
                className="flex-1 bg-black border border-zinc-800 rounded-xl p-2 text-xs text-white focus:outline-none focus:border-cyan-500 font-mono"
              />
              <button
                type="submit"
                className="px-4 py-2 bg-cyan-500 text-black font-bold text-xs rounded-xl shadow"
              >
                Send
              </button>
            </form>
      ...
      <ToolFooter
        title="Sovereign Communications Engine"
        details="Client-side OpenPGP text encryption combined with decentralized Matrix/P2P message transport."
        disclaimer="Zero central server logs."
      />
    </div>
  );
}
