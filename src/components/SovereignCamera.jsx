...
export function SovereignCamera({ onNavigate, navigateTo, onScanSuccess, forcedMode }) {
  const navHandler = onNavigate || navigateTo;
  const videoRef = useRef(null);
  const [mode, setMode] = useState(forcedMode || "Photo"); 
  ...
  useEffect(() => {
    ...
    return () => cancelAnimationFrame(scanFrame);
  }, [mode, scannedResult, onScanSuccess]);
  ...
  const handleAction = () => {
    ...
    if (mode === "Video") {
      ...
      // Delay slightly to allow the UI to render the 'Encrypting' state before locking the main thread
      setTimeout(() => {
        const canvas = document.createElement("canvas");
        canvas.width = videoRef.current.videoWidth;
        canvas.height = videoRef.current.videoHeight;
        ...
        const fileName = `stealth_cap_${Date.now()}.aes`;
        ...
        Filesystem.writeFile({
          ...
          alert("📸 Encrypted Photo secured in Vault.");
        }).catch(e => {
          setIsEncrypting(false);
  ...
  const handleSaveVideo = async () => {
    ...
    reader.onloadend = async () => {
      ...
      // Offload to a timeout to prevent UI freezing
      setTimeout(async () => {
        const encryptedData = CryptoJS.AES.encrypt(base64Data, masterKey).toString();
        const fileName = `stealth_vid_${Date.now()}.aes`;
        ...
        setIsEncrypting(false);
        alert("🎥 Encrypted Video secured in Vault.");
        setRecordedBlob(null);
      }, 50);
    };
  };
    ...
    <div className="fixed inset-0 bg-black flex flex-col justify-between z-50 select-none">
      ...
      {isEncrypting && (
        <div className="fixed inset-0 z-[99999] bg-black/90 backdrop-blur-xl flex flex-col items-center justify-center">
           ...
           <h2 className="text-emerald-500 font-black tracking-[0.2em] uppercase text-lg animate-pulse">Encrypting Payload</h2>
           <p className="text-zinc-400 font-mono text-[10px] mt-2">Wrapping media in AES-256...</p>
        </div>
      )}
      ...
      <div className="absolute top-0 left-0 right-0 flex justify-between items-center p-4 z-20">
        <button onClick={() => navHandler('home')} className="bg-zinc-900/80 border border-zinc-700 text-white text-xs font-bold px-4 py-2 rounded-full backdrop-blur-md active:scale-95 shadow-lg">
          X Exit
        </button>

        <div className="flex gap-2">
          <button onClick={() => setBlackout(true)} className="bg-zinc-900/80 border border-zinc-700 text-white text-xs font-bold px-4 py-2 rounded-full backdrop-blur-md active:scale-95 shadow-lg">
            Dim
          </button>
          <button onClick={() => setNvgMode(!nvgMode)} className={`border text-xs font-bold px-4 py-2 rounded-full backdrop-blur-md active:scale-95 shadow-lg ${nvgMode ? 'bg-emerald-500/80 border-emerald-400 text-black' : 'bg-zinc-900/80 border-zinc-700 text-white'}`}>
            🌙 NVG
          </button>
          <button onClick={toggleTorch} className={`border text-xs font-bold px-4 py-2 rounded-full backdrop-blur-md active:scale-95 shadow-lg ${torch ? 'bg-yellow-500/80 border-yellow-400 text-black' : 'bg-zinc-900/80 border-zinc-700 text-white'}`}>
            ⚡ Flash
          </button>
          <button onClick={() => setFacingMode(prev => (prev === "environment" ? "user" : "environment"))} className="bg-zinc-900/80 border border-zinc-700 text-white text-xs font-bold px-4 py-2 rounded-full backdrop-blur-md active:scale-95 shadow-lg">
            🔄 Flip
          </button>
        </div>
      </div>
          ...
          <div className="absolute inset-x-6 bottom-32 bg-zinc-950/95 border-2 border-red-500/80 rounded-2xl p-5 z-30 shadow-[0_0_35px_rgba(239,68,68,0.3)] backdrop-blur-xl animate-fadeIn">
            <div className="flex justify-between items-center mb-3">
              <span className="text-[10px] font-black text-red-500 tracking-widest uppercase flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-red-500 animate-ping"></span> Video Payload Captured
              </span>
              <span className="text-[10px] font-mono text-zinc-400">{(recordedBlob.size / (1024 * 1024)).toFixed(2)} MB</span>
            </div>
            <div className="flex gap-2">
              <button onClick={handleSaveVideo} className="flex-1 bg-red-600 hover:bg-red-500 text-white font-black text-xs py-3 rounded-xl uppercase tracking-wider active:scale-95 shadow-lg">Save to Vault</button>
              <button onClick={() => setRecordedBlob(null)} className="bg-zinc-800 hover:bg-zinc-700 text-white font-bold text-xs px-5 py-3 rounded-xl uppercase tracking-wider active:scale-95">Discard</button>
            </div>
          </div>
        )}
          ...
          <div className="absolute inset-x-6 bottom-32 bg-zinc-950/95 border-2 border-emerald-500/80 rounded-2xl p-5 z-30 shadow-[0_0_35px_rgba(16,185,129,0.3)] backdrop-blur-xl animate-fadeIn">
            <div className="flex justify-between items-center mb-2">
              <span className="text-[10px] font-black text-emerald-400 tracking-widest uppercase flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping"></span> Payload Extracted
              </span>
            ...
            <div className="max-h-36 overflow-y-auto bg-black/80 border border-zinc-800 rounded-xl p-3 mb-3 font-mono text-xs text-zinc-200 select-all break-all">{scannedResult}</div>
            <div className="flex gap-2">
              <button onClick={() => { navigator.clipboard.writeText(scannedResult); setCopyToast(true); setTimeout(() => setCopyToast(false), 2000); }} className="flex-1 bg-emerald-500 hover:bg-emerald-400 text-black font-black text-xs py-2.5 rounded-xl uppercase tracking-wider active:scale-95">Copy Again</button>
              <button onClick={() => setScannedResult(null)} className="bg-zinc-800 hover:bg-zinc-700 text-white font-bold text-xs px-5 py-2.5 rounded-xl uppercase tracking-wider active:scale-95">Clear</button>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/90 to-transparent pt-12 pb-8 flex flex-col items-center gap-6 z-20">
        <div className="flex gap-4 bg-zinc-900/60 px-6 py-2 rounded-full backdrop-blur-md border border-zinc-800">
          {[1, 2, 3].map((lvl) => (
            <button key={lvl} onClick={() => applyZoom(lvl)} className={`text-xs font-bold transition-all ${Math.round(zoom) === lvl ? 'text-emerald-400 scale-110' : 'text-zinc-500 hover:text-white'}`}>{lvl}x</button>
          ))}
        </div>

        {mode !== "QR" ? (
          <button onClick={handleAction} disabled={isExtracting} className={`w-16 h-16 rounded-full border-4 border-white flex items-center justify-center active:scale-90 transition-transform shadow-[0_0_20px_rgba(0,0,0,0.5)] ${isExtracting ? 'opacity-50 cursor-wait' : ''}`}>
            <div className={`w-12 h-12 rounded-full ${mode === "Video" ? isRecording ? 'bg-red-600 animate-pulse' : 'bg-red-500' : mode === "OCR" ? 'bg-cyan-500' : 'bg-white'}`} />
          </button>
        ) : (
