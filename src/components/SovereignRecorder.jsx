...
export function SovereignRecorder({ onNavigate }) {
    ...
    try {
      ...
      if (scan && scan.files) {
        const parsed = scan.files
          .filter(f => { const name = typeof f === 'string' ? f : f.name; return name.endsWith('.aac') || name.endsWith('.webm') || name.endsWith('.mp4'); })
  ...
  const loadPlaylists = () => { try { const saved = localStorage.getItem('sovereign_record_playlists'); if (saved) setPlaylists(JSON.parse(saved)); } catch (e) {} };
  const savePlaylists = (updated) => { setPlaylists(updated); localStorage.setItem('sovereign_record_playlists', JSON.stringify(updated)); };
  ...
  const renameRecord = async (record) => {
    const newName = window.prompt("Enter new name (without extension):", record.name.split('.')[0]);
    if (!newName) return;
    try {
      await Filesystem.rename({ from: `${FOLDER_PATH}/${record.name}`, to: `${FOLDER_PATH}/${newName}.aac`, directory: Directory.Documents });
      loadRecords();
    } catch (e) { alert("Failed to rename. Ensure name has no special characters."); }
  };
  ...
  const addToPlaylist = (playlistId, record) => {
    const updated = playlists.map(p => {
      if (p.id === playlistId && !p.tracks.some(t => t.path === record.path)) { return { ...p, tracks: [...p.tracks, record] }; }
      return p;
    });
      ...
      {currentPlayback && (
        <div className="bg-zinc-900/90 border border-rose-500/40 p-4 rounded-3xl flex justify-between items-center shadow-2xl animate-fadeIn">
          ...
          <div className="flex gap-2">
             ...
             <button onClick={() => { setCurrentPlayback(null); setIsPlaying(false); if(audioPlayerRef.current) audioPlayerRef.current.pause(); }} className="w-10 h-10 bg-zinc-800 text-zinc-400 rounded-full flex items-center justify-center text-xs border border-zinc-700 active:scale-95">⏹</button>
          </div>
        </div>
      )}
                ...
                {showPlaylistMenu === idx && (
                   <div className="absolute right-0 top-full mt-2 w-48 bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl z-50 overflow-hidden animate-fadeIn">
                      ...
                      <div className="max-h-40 overflow-y-auto">
                         ...
                         {playlists.map(pl => (
                            <button key={pl.id} onClick={() => addToPlaylist(pl.id, rec)} className="w-full text-left px-4 py-3 text-xs font-bold text-white hover:bg-rose-900/40 transition-colors border-b border-zinc-800/50 last:border-0 truncate">{pl.name}</button>
                         ))}
                      </div>
                   </div>
                )}
