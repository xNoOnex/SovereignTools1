...
export function SovereignBrowser({ onNavigate }) {
    ...
    const ripPayload = async (targetUrl, isManual = false) => {
        ...
        try {
            ...
            // =================================================================
            // STREAMING MODE: For 4-Hour Movies & Massive Payloads (Zero RAM)
            // =================================================================
            if (lowRamMode) {
            ...
            // =================================================================
            // STANDARD MODE: For Images & Fast Clips (Full RAM Buffer)
            // =================================================================
            let mediaBuffer;
            let ext = 'bin';
    ...
    return (
        <div className="flex flex-col h-full bg-black text-zinc-300 animate-fadeIn relative">
            <div className="bg-zinc-950 border-b border-zinc-800 shrink-0 p-3 flex flex-col gap-2 z-40">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest hidden sm:inline">WebKit Isolated</span>
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto py-1 no-scrollbar">
                    {tabs.map((tab) => (
                        <div key={tab.id} onClick={() => setActiveTabId(tab.id)} className={`flex items-center gap-2 px-3 py-1 rounded-lg text-xs font-mono cursor-pointer border transition-all shrink-0 max-w-[140px] ${tab.id === activeTabId ? 'bg-zinc-900 border-cyan-500/50 text-cyan-400' : 'bg-black border-zinc-800 text-zinc-500 hover:border-zinc-700'}`}>
                            <span className="truncate">{tab.title}</span>
                            <span onClick={(e) => closeTab(tab.id, e)} className="text-zinc-600 hover:text-rose-400 font-bold">×</span>
                        </div>
                    ))}
                    <button onClick={createNewTab} className="px-2.5 py-1 bg-zinc-900 hover:bg-zinc-800 text-cyan-400 border border-zinc-800 rounded-lg text-xs font-bold shrink-0">+</button>
                </div>
                ...
                {!isUnlocked ? (
                    ...
                    <div className="flex gap-2 relative animate-fadeIn">
                        <button onClick={goBack} disabled={!activeTab || activeTab.history.length === 0} className={`px-3 rounded-md border transition-all flex items-center justify-center font-bold ${activeTab && activeTab.history.length > 0 ? 'bg-zinc-900 border-zinc-700 text-cyan-400 active:scale-95' : 'bg-zinc-950 border-zinc-900 text-zinc-700 opacity-50 cursor-not-allowed'}`}>←</button>
                        ...
                        <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleNavigate()} placeholder="Enter web address..." className="flex-grow bg-black border border-zinc-700 rounded-md px-3 py-2.5 text-sm font-mono text-zinc-300 focus:outline-none focus:border-cyan-500" />
                        <button onClick={() => handleNavigate()} className="px-4 bg-cyan-900/30 border border-cyan-900/50 text-cyan-400 font-bold text-xs tracking-widest active:scale-95 transition-all">GO</button>
            ...
            {showSettings && isUnlocked && (
                <div className="absolute top-[170px] left-3 right-3 bg-zinc-900 border border-zinc-700 rounded-xl p-4 shadow-2xl z-50 flex flex-col gap-5">
                    
                    {/* DIRECT PAYLOAD SNIPER */}
                    <div className="flex flex-col gap-2">
                        <span className="text-xs font-bold uppercase tracking-widest text-emerald-400">🎯 Direct Payload Sniper</span>
                        <p className="text-[10px] text-zinc-500 font-mono leading-tight mb-1">Paste a direct media link (.jpg, .m3u8, .mp4) to bypass browser rendering and rip directly to Vault.</p>
                        <div className="flex gap-2">
                            <input 
                                ...
                                className="flex-grow bg-black border border-emerald-900/50 rounded-md px-3 py-2 text-xs font-mono text-emerald-300 focus:outline-none focus:border-emerald-500" 
                            />
                            <button 
                                onClick={() => ripPayload(sniperUrl, true)}
                                disabled={!sniperUrl || isExtracting}
                                className={`px-4 font-black text-xs tracking-widest rounded-md uppercase transition-all ${!sniperUrl || isExtracting ? 'bg-zinc-800 text-zinc-600 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-500 text-black shadow-[0_0_15px_rgba(16,185,129,0.3)] active:scale-95'}`}
                            >
                                RIP
                            </button>
                        </div>
                    ...
                    {/* GOD TIER STREAMING TOGGLE */}
                    <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                            <span className="text-xs font-bold uppercase tracking-widest text-cyan-400">🌊 Low-RAM Streaming Mode</span>
                            <span className="text-[9px] text-zinc-500 font-mono uppercase mt-0.5">Chunk-by-Chunk AES for 4hr+ Movies</span>
                        </div>
