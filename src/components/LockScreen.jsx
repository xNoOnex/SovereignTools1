...
export function LockScreen({ onUnlock }) {
  ...
  return (
    <div className="fixed inset-0 bg-black z-50 flex flex-col items-center justify-between p-6 font-sans select-none relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 opacity-[0.05] pointer-events-none" style={{ backgroundImage: "url('/Appicon.jpg')", backgroundSize: 'cover', backgroundPosition: 'center' }}></div>

      {/* Top Section: Logo & Disclaimers */}
      <div className="flex flex-col items-center mt-12 space-y-4 z-10 w-full">
        <div className="w-24 h-24 rounded-3xl overflow-hidden shadow-2xl shadow-zinc-900/50 border border-zinc-800 bg-zinc-950">
      ...
      <div className="z-10 w-full flex flex-col items-center pb-4">
        <button onClick={() => setShowInfo(!showInfo)} className="text-[10px] uppercase font-bold text-zinc-400 hover:text-white border border-zinc-700 px-5 py-2.5 rounded-full backdrop-blur bg-zinc-900/50 transition-colors shadow-lg active:scale-95">
          {showInfo ? 'Close Info' : 'ℹ️ About Sovereign'}
        </button>
        
        {showInfo && (
          <div className="absolute bottom-20 left-4 right-4 bg-zinc-950/95 backdrop-blur-xl border border-zinc-800 p-6 rounded-3xl shadow-2xl animate-fadeIn space-y-4 text-center z-20">
            ...
            <p className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest pt-3 border-t border-zinc-900">
