...
export function LockScreen({ onUnlock }) {
  ...
  const handleKeyPress = async (num) => {
    ...
    // 4. Failed Attempt Evaluation
    if (newStr.length >= 4) {
      const wipeLimit = localStorage.getItem('sovereign_wipe_limit') || "None";
      ...
      if (wipeLimit !== "None" && attempts >= parseInt(wipeLimit)) {
        executeProtocol();
        return;
      }
    ...
    <div className="fixed inset-0 bg-black z-50 flex flex-col items-center justify-between p-8 font-sans select-none relative overflow-hidden" style={{ backgroundImage: 'linear-gradient(rgba(0,0,0,0.85), rgba(0,0,0,0.95)), url("/AppIcon.jpg")', backgroundSize: 'cover', backgroundPosition: 'center' }}>
      ...
      <div className="flex flex-col items-center z-10 w-full max-w-[280px]">
        ...
        <div className="grid grid-cols-3 gap-5 w-full">
          ...
          <button onClick={() => setPinEntry(prev => prev.slice(0, -1))} className="h-16 bg-black/40 border border-zinc-800 hover:border-zinc-700 rounded-xl text-xl font-bold text-zinc-400 active:scale-95 transition-all flex items-center justify-center">
            ⌫
          </button>
        </div>
      </div>
        ...
        {showInfo && (
          <div className="absolute bottom-24 left-6 right-6 theme-glass-panel backdrop-blur-xl p-6 rounded-2xl shadow-2xl animate-fadeIn text-center border-l-4 border-l-[var(--accent-text)]">
            ...
            <p className="text-[10px] text-zinc-300 font-mono leading-relaxed text-justify">
              This terminal is disconnected from telemetry networks. Cryptographic keys are confined to volatile RAM. Unauthorized access attempts are logged locally.
            </p>
          </div>
        )}
      </div>
